<div class="rounded bg-white">
    ...
    <div class="p-3">
        <form action="{{ route('admin.edit_news') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="id" value="{{ $detail_news->id }}">
            <div class="mb-3">
                <label class="form-label">Chọn danh mục</label>
                <select name="chondanhmuc" class="form-select" aria-label="Default select example">
                    @foreach ($show_category as $i)
                        <option {{ $detail_news->idcategory == $i->id ? 'selected' : '' }} value="{{ $i->id }}">{{ $i->ten }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-3">
                ...
                <input value="{{ $detail_news->title }}" name="tieude_baiviet" type="text" class="form-control" id="tieude" placeholder="Nhập tiêu đề bài viết">
            </div>

            <div class="mb-3 d-flex">
                <div class="pe-5 me-5 border-black border-end">
                    <label class="form-label">avatar đang tồn tại</label>
                    <div style="max-width: 200px;">
                        <img class="w-100" src="{{ asset($detail_news->avatar) }}" alt="">
                        <input type="hidden" name="anhcu" value="{{ asset($detail_news->avatar) }}">
                    </div>
                </div>
                <div>
                    <label class="form-label">Chọn avatar mới</label>
                    <div class="drop-zone">
            ...
            <div class="mb-3">
                <div class="container-richtext">
                    ...
                    <div id="text-input" class="d-flex flex-column" contenteditable="true">
                        @php
                            echo $detail_news->content
                        @endphp 
                    </div>
            ...
            <div class="mb-3">
                ...
                <select name="trangthai_baiviet" class="form-select" aria-label="Default select example">
                    <option {{ $detail_news->status == 0 ? 'selected' : '' }} value="0">Hiển thị</option>
                    <option {{ $detail_news->status == 1 ? 'selected' : '' }} value="1">Ẩn</option>
                </select>
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="noibat_baiviet" class="form-select" aria-label="Default select example">
                    <option {{ $detail_news->hots == 0 ? 'selected' : '' }} value="0">Bình thường</option>
                    <option {{ $detail_news->hots == 1 ? 'selected' : '' }} value="1">Nổi bật</option>
                </select>
            </div>
